...
void Model::draw(std::vector<DirectionLight> lights)
{
	...
	//std::vector<glm::mat4> LVP;
	for each (DirectionLight var in lights)
	{
		Projection = getOrthoProject(var.dir);
		View = getOrthoView(var.dir);
		

	}
	Projection = getOrthoProject(lights[0].dir);
	View = getOrthoView(lights[0].dir);
...
void Model::DrawToShadowMap(glm::vec3 orthoDir)
{
//	this->CalcBones(this->getTime());
	glm::mat4 Projection = getOrthoProject(orthoDir);
	glm::mat4 View = getOrthoView(orthoDir);
...
void Model::setSoundEngine(ISoundEngine * engine)
{
	if (this->sounds == nullptr) {
		this->sounds = new Sound();
	}
	this->sounds->setEngine(engine);
}

void Model::addSound(std::string key, std::string path)
{
}

void Model::setWind(Wind * w)
{
	this->wind = w;
}
